const UserRegisterHandler = (app, db) => {
    app.post("/reg/usr", (req, res) => {
      const { name, email, address, date_of_birth, gender, blood_group, contact_number, username, password } = req.body;
        ...
        db.query(sqlCheck, [username, email], (err, result) => {
          ...
          db.query(sqlGetMaxUserId, (err, result) => {
            ...
            db.query(sqlInsertCredentials, [newUserId, username, password], (err, result) => {
              ...
              const sqlInsertUsers = "INSERT INTO users (user_id, name, email, address, date_of_birth, role, gender, blood_group, contact_number) VALUES (?, ?, ?, ?, ?, 'user', ?, ?, ?)";
              db.query(sqlInsertUsers, [newUserId, name, email, address, date_of_birth, gender, blood_group, contact_number], (err, result) => {
                if (err) {
                  db.rollback(() => {
                    res.send({ message: "Error in registration: " + err });
                  });
