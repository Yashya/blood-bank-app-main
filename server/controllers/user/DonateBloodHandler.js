      ...
      const handleDonation = (donorId) => {
        ...
        db.query(fetchBloodGroup, [donorId], (err, result) => {
          if (err) {
            ...
            const fetchLastSampleId = "SELECT sample_id FROM blood_samples ORDER BY sample_id DESC LIMIT 1";
            db.query(fetchLastSampleId, (err, result) => {
              if (err) {
                ...
                const newSampleId = `SAM${numericPartSample}`;
  
                // Fetch the last audit_log id and generate a new one
                const fetchLastAuditId = "SELECT id FROM audit_log ORDER BY id DESC LIMIT 1";
                db.query(fetchLastAuditId, (err, result) => {
                  if (err) {
                    ...
                    const newAuditId = `AU${numericPartAudit}`;
